...
def generate_env_file():
    ...
    print(
        "Only the OpenAI API key and the backup file path are required and has no default value.\n"
    )
    ...
    if not openai_api_key or not bak_file_path:
        print(
            "ERROR: OPENAI_API_KEY and BAK_FILE_PATH are required for the .env file to be created."
        )
        sys.exit(1)
