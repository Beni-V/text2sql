    ...
    def _create_schema_documents(self, schema: dict) -> list[Document]:
        ...
        for table_name, table_info in schema.items():
            ...
            for col_name, col_info in columns.items():
                ...
                is_nullable = "nullable" if col_info.get("is_nullable") == "YES" else "not nullable"
            ...
            content = f"Table: {table_schema_name}.{table_name}\nColumns:\n" + "\n".join(column_info)
            metadata = {
        ...
        for table_name, table_info in schema.items():
            ...
            for fk in relationships.get("foreign_keys", []):
                content = (f"Foreign Key Relationship: {table_name}.{fk.get('column')} "
            ...
            for ref in relationships.get("referenced_by", []):
                content = (f"Reference Relationship: {ref.get('table')}.{ref.get('column')} "
